...
//cwe
bool IniFile::getBool(const string& key, bool def) const
{
	auto iter = m_globalGroup.find(key);
	if (iter == m_globalGroup.end())
		return def;
...
//cwe
int IniFile::getInt(const string& key, int def) const
{
	auto iter = m_globalGroup.find(key);
	if (iter == m_globalGroup.end())
		return def;

	//radix = 10, i never touched it anyways
	return (int)std::stoll(iter->second, nullptr, 10);
}
