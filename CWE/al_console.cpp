...
void Console_Display(ObjectMaster* a1)
{
	//if (ScaleObjectMaster_XYZ(a1, 3.0f, 3.0f, 1.0f)) 
	{
		...
		njTranslate(NULL, -2.5f, -1.5, 0.0);
		njTranslateEx(&a1->Data1.Entity->Position);
		//njRotateY(0, a1->Data1->Rotation.y);
		//njScale(0, 1, 1, 1);
		chCnkDrawObject(&wii_main);

		if (a1->Data1.Entity->Action == 0) {
			njTranslate(NULL, -2.5f, 0, 0);
			njRotateY(NULL, 0x1000);
			njScale(NULL, 2, 2, 2);
			chCnkDrawObject(&wii_remote);
		}
...
ObjectMaster* PlaceConsole(NJS_VECTOR* pos)
{
	...
	LoadTextureList((char*)"al_wii", &AL_WII_TEXLIST);
	tv = LoadObject(4, "AL_Wii", Console_Load, LoadObj_Data1);
	tvData = tv->Data1.Entity;
	consolePtr = tv;
	if (tv)
	{
		tvData->Position = *pos;
