...
#define GET_SPEECHWK(tp) ((speechwk*)(tp->awp))
...
struct speech_bubble_entry_new {
	...
	NJS_CNK_OBJECT** pObjList; // optional
...
static NJS_CNK_OBJECT DrumFix = {
	NJD_EVAL_UNIT_POS | NJD_EVAL_UNIT_ANG | NJD_EVAL_UNIT_SCL,
	((NJS_CNK_OBJECT*)0x11C62C4)->model
};

extern NJS_CNK_OBJECT object_alo_accordion;
extern NJS_CNK_OBJECT object_alo_guitar;
static NJS_CNK_OBJECT* MusicObjList[] = {
	(NJS_CNK_OBJECT*)0x11C5B3C, // bell
	(NJS_CNK_OBJECT*)0x11C7A9C, // castanas or whatever they're called
	(NJS_CNK_OBJECT*)0x11C8964, // cymbal
	&DrumFix,				// drum
	(NJS_CNK_OBJECT*)0x11C73A4, // flute
	(NJS_CNK_OBJECT*)0x11C505C, // maracas
	(NJS_CNK_OBJECT*)0x11C830C, // trumpet
	(NJS_CNK_OBJECT*)0x11C9AE4, // 
};

static speech_bubble_entry_objtex_pair MusicObjListCWE[] = {
	{&object_alo_guitar, texlist_cwe_object},
...
static speech_bubble_entry_new entries_new[NB_SPEECH_ENTRY] = {
	...
	{-1, (NJS_CNK_OBJECT**)0x012E58FC, (NJS_TEXLIST*)0x1366AD4, NULL, 0, -75, 1.5f}, // MEDAL (big medals, offset by 1, ids start at 0)
	{-1, (NJS_CNK_OBJECT**)0x0136184C, &AL_TOY_TEXLIST, NULL, 0, 0, 0.85f}, // LTOY (race LToyRewards list)
	//{-1, (NJS_CNK_OBJECT**)0x0136184C, &AL_TOY_TEXLIST, 0, 0, 0.85f}, // LTOY_CWE
	{-1, (NJS_CNK_OBJECT**)0x1361888, &AL_TOY_TEXLIST, NULL, SPEECH_BUBBLE_SKIP_PER_INDEX, 0, 0.85f}, // STOY
	{-1, MusicObjList, &AL_TOY_TEXLIST, NULL, 0, -100, 3}, // Music
	{-1, NULL, NULL, MusicObjListCWE, 0, -100, 3}, // MusicCWE
	{-1, (NJS_CNK_OBJECT**)GetDataDllProcAddr("MayuObjectList"), &AL_BODY, NULL, 0, 75, 1} // Cocoon
};

static speech_bubble_entry entries[NB_SPEECH_ENTRY] = {
	{NULL, NULL, ALW_CATEGORY_FRUIT, ChaoFruit_SquareFruit, 0},
	{NULL, NULL, ALW_CATEGORY_FRUIT, ChaoFruit_RoundFruit, 0},
	{NULL, NULL, ALW_CATEGORY_FRUIT, ChaoFruit_TriangleFruit, 0},

	{(NJS_CNK_OBJECT*)0x0130111C, (NJS_TEXLIST*)0x01366AFC, ALW_CATEGORY_FRUIT, ChaoFruit_TriangleFruit, SPEECH_BUBBLE_TRANSLATE, {0, -2.547241f, -0.154359f}},
};
...
static task* AL_SpeechBubble_CreateMockChao(CHAO_SAVE_INFO* chaoData) {
	task* pChao = CreateElementalTask(0, LEV_4, [](task*) {}, "MockChao");
	...
	work->pParamGC = (CHAO_PARAM_GC*)ALLOC(CHAO_SAVE_INFO);
	work->IsParamCopy = TRUE;
	memcpy(work->pParamGC, chaoData, sizeof(CHAO_SAVE_INFO));

	AL_MotionInit(pChao);
	AL_ShapeInit(pChao);
	AL_FaceInit(pChao);
	sub_566B80(pChao);
	AL_ChaoAccessoryConversion(GET_CWEPARAM(pChao));
	AL_ChaoAccessoryMainCheck(pChao);
	
	AL_SetMotionLink(pChao, 0);
	MotionControl(&work->MotionCtrl);
...
static NJS_CNK_MODEL* FindFirstModel(NJS_CNK_OBJECT* obj) {
	while (obj) {
...
static void AL_SpeechBubble_RenderItem(speech_bubble_entry_new& entry, Uint32 entryID) {
	...
	const auto& obj = entry.pObjList ? entry.pObjList[entryID] : ObjectRegistry::Get(entry.category)->GetObj(entryID);
	const auto& texlist = entry.pTexlist ? entry.pTexlist : ObjectRegistry::Get(entry.category)->GetTex(entryID);
	const auto& mdl = FindFirstModel(obj);

	NJS_POINT3 center;
	const float rad = CalcMaxRadius(obj, center);
	...
	LoadNewShaders();
	OrthoDrawBegin();
	OrthoScreenTranslate(320 - center.x, 240 - center.y, 8.f / rad);
	
	static int ang = 0;
	ang += 0x100;
	//njRotateY(NULL, ang);
	
	//njScale(NULL, 1 / rad, 1 / rad, 1 / rad);
...
static NJS_POINT3 AL_SpeechBubble_GetPosition(task* pChao, eSpeechPos position) {
	...
	switch (position) {
		case SPEECH_POS_LEFT:
			sub_426CC0(_nj_current_matrix_ptr_, &viewPosition, &GET_CHAOWK(pChao)->Shape.HeadPos, 0);
			viewPosition.x += -3;
			break;
...
static void AL_SpeechBubbleDisplayer(task* tp) {
	...
	NJS_POINT3 position = AL_SpeechBubble_GetPosition(tp->ptp, work->position);
	...
	njScale(NULL, sizeMul, sizeMul * DisplayResolutionY / DisplayResolutionX, 1);
	njDrawTexture3DExSetData(SpeechBubbleTex, 4);
	njPopMatrixEx();
...
task* AL_SpeechBubbleCreate(task* pChao, CHAO_SAVE_INFO* pChaoData, Uint32 entryType, Uint32 entry, eSpeechPos position, Uint32 spawnTimer, Uint32 aliveTimer) {
	task* obj = CreateChildTask(0, AL_SpeechBubbleExecutor, pChao);

	obj->disp_dely = AL_SpeechBubbleDisplayer;
	obj->dest = AL_SpeechBubbleDestructor;
