...
PS_IN main(VS_INPUT input)
{
    ...
    float3 normal = normalize(origNormal);
    ...
    for (int index = 0; index < 4; ++index)
    {
        ...
        if (g_LightFuncEnable[0].x > 0.5f && g_LightMask[index].x > 0.5f)
        {
            ...
            finalColor.xyz += finalDiffuse * g_LightColor[index].xyz;
        }
    ...
    {
        output.TexCoord.xy = 0.5 - normal.xy * 0.5;
        output.TexCoord.z = 0;
    }
